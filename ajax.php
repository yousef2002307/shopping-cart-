...
$total_records = $stmt1->rowCount();
    ...
    $stmt1 = $db->prepare("SELECT * FROM `products` WHERE name Like '%$val%'");
    $stmt1->execute();
    $total_records = $stmt1->rowCount();
...
$stmt = $db->prepare("SELECT * FROM `products` WHERE quantity > 0 LIMIT $offset,$total_records_per_page ");
    ...
    $stmt = $db->prepare("SELECT * FROM `products` WHERE name Like '%$val%' AND quantity > 0 LIMIT $offset,$total_records_per_page ");
 }
$stmt->execute();
